...
import { Mail, DollarSign, CheckCircle2, ShieldAlert, ArrowRight } from 'lucide-react';
...
export const InteractiveDashboardDemo: React.FC = () => {
  ...
  // Steps:
  // 0: Waiting/Detecting
  // 1: Mail detected notification (slowed down)
  // 2: Counting leak up to $100 (slowed down)
  // 3: Send Invoice button appears (slowed down)
  // 4: Cursor gliding to Send Invoice (slowed down)
  // 5: Clicking Send Invoice (slowed down)
  // 6: Success / Recovered full screen
  // 7: Wait & reset

  const [leakValue, setLeakValue] = useState(0);
  ...
  // Main animation sequence controller (significantly slowed down)
  useEffect(() => {
    ...
    if (step === 0) {
      ...
      if (isInView) {
        timeout = setTimeout(() => {
          setStep(1);
        }, 2200); // Wait longer before starting
      }
    }
    else if (step === 1) {
      // Mail detected state. Wait a bit, then start leak calculations.
      timeout = setTimeout(() => {
        setStep(2);
      }, 2000); // Slowed down from 1000
    }
    else if (step === 2) {
      // Count leak up from 0 to 100
      let start = 0;
      const end = 100;
      const duration = 2200; // ms (Slower counter speed)
      ...
      // Send Invoice button becomes active, cursor appears and glows
      setCursorX(82);
      setCursorY(72);
      setCursorOpacity(1);
      ...
      timeout = setTimeout(() => {
        setStep(4);
      }, 1200); // Slowed down from 300
    }
    else if (step === 4) {
      // Smoothly glide cursor to the "Send Invoice" button coordinate
      // Centered precisely on the button in the right card of the frame (X: 74%, Y: 84%)
      setCursorX(74);
      setCursorY(84);
      ...
      timeout = setTimeout(() => {
        ...
        timeout = setTimeout(() => {
          setStep(6);
        }, 600); // Slower transition after click
      }, 400);
    }
    else if (step === 6) {
      ...
      timeout = setTimeout(() => {
        setStep(7);
      }, 3500); // Hold success state longer so user can read it
    }
    else if (step === 7) {
      // Fade out and reset
      timeout = setTimeout(() => {
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full flex items-center justify-center">
      <AnimatePresence mode="wait">
        {step !== 7 && (
          <motion.div
            id="leak-dashboard-card"
            key="dashboard-window"
            initial={{ opacity: 0, scale: 0.95, y: 15 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            exit={{ opacity: 0, scale: 0.92, y: -40, filter: 'blur(4px)' }}
            transition={{ type: 'spring', damping: 25, stiffness: 180 }}
            className="w-full max-w-[480px] aspect-[0.88] xs:aspect-[0.96] sm:aspect-[1.25] bg-white rounded-2xl shadow-[0_24px_70px_rgba(0,0,0,0.08),_0_1px_3px_rgba(0,0,0,0.02)] border border-neutral-150 overflow-hidden flex flex-col font-sans select-none text-left text-neutral-800 relative"
          >
            {/* macOS Title Bar */}
                  ...
                  <div className="bg-[#f8f9fa] border border-neutral-200/60 rounded-xl p-3.5 flex flex-col justify-between text-left h-[104px] shadow-[0_1px_2px_rgba(0,0,0,0.01)]">
                    ...
                    <div className="flex flex-col">
                      <span className="text-3xl font-semibold tracking-tight text-red-500 font-mono flex items-baseline">
                        <DollarSign className="w-5 h-5 self-center text-red-500 -mr-0.5" />
                        {leakValue}
                      </span>
                      <span className="text-[9px] text-neutral-500 mt-1">
                        {leakValue > 0 ? "⚠️ Leak actively flagged" : "0 leaks detected"}
                      </span>
                    </div>
                  </div>

                  {/* Send Invoice Action Card */}
